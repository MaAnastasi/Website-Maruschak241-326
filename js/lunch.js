...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (isMatch) {
    ...
    const isSpecialCombo = 
      selectionState.soup?.keyword === "chicken_noodle" &&
      selectionState.main?.keyword === "mushroom_pasta" &&
      selectionState.drink?.keyword === "black_tea";
...
function calculateComboPrice() {
  // Находим блюда в комбо
  const chickenSoup = dishes.find(dish => dish.keyword === "chicken_noodle");
  const mushroomPasta = dishes.find(dish => dish.keyword === "mushroom_pasta");
  const blackTea = dishes.find(dish => dish.keyword === "black_tea");
  
  if (!chickenSoup || !mushroomPasta || !blackTea) return 0;
  
  const totalPrice = chickenSoup.price + mushroomPasta.price + blackTea.price;
  const discount = totalPrice * 0.15; // 15% скидка
  return Math.round(totalPrice - discount);
}

// Обновляем цены комбо при загрузке
document.addEventListener("DOMContentLoaded", () => {
  const comboPrice = calculateComboPrice();
  const totalPrice = 250 + 340 + 120; // Цены из dishes-data.js
  ...
  if (selectComboBtn) {
    selectComboBtn.addEventListener('click', () => {
      ...
      if (!chickenSoup || !mushroomPasta || !blackTea) {
      ...
      const comboPrice = calculateComboPrice();
      const totalPrice = 250 + 340 + 120;
      
      overlay.innerHTML = `
        <div class="notification-popup">
          <p>Вы выбрали специальное комбо!</p>
          <p>Куриный суп + Паста с грибами + Черный чай</p>
